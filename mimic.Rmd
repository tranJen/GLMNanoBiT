...
#then calculate the ratio of Ham = Yes to Ham = No for each sample and each trial
pertrans <- data %>%
...
#plot
ggplot(pertrans, aes(x = Sample, y = Ratio*100, color = Sample)) +
...
#RLU
```{r}
data['Trial1'] <- log10(data['Trial1'])
data['Trial2'] <- log10(data['Trial2'])
data['Trial3'] <- log10(data['Trial3'])
...
```{r}
ggplot(means, aes(x = Sample, y = mean_value, fill = Ham)) +
  geom_bar(stat = "identity", position = "dodge", color = "black", width = 0.5) +
  geom_errorbar(aes(ymin = mean_value - std_error, ymax = mean_value + std_error), position = position_dodge(width = 0.9), width = 0.1) +
  xlab("Sample") +
  ylab("RLU") +
