...
```{r}
data_JE<- arrange(data_JE, pos)
data_JE$pos <- as.character(data_JE$pos)
```


```{r}
# Convert pos to a factor with levels including "25"
data_JE$pos <- as.factor(data_JE$pos)
levels(data_JE$pos) <-c("25", levels(data_JE$pos)[-which(levels(data_JE$pos) == "25")])

# Set "25" as the reference level
data_JE$pos <- relevel(data_JE$pos, ref = "25")
...
desired_order <- c(2:25)  # Modify this as needed
